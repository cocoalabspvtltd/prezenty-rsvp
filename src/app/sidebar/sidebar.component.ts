...
export class SidebarComponent implements OnInit {
  ...
  handleEventDetail() {
    ...
    this.apiService.getEventDetail(this.eventId).subscribe((res: any) => {
      if (res) {
        ...
        let longMonth = this.date.toLocaleString("en-us", {
          month: "short",
        }); /* June */
        ...
        const eventday = new Date(this.eventDay).toLocaleString("en-us", {
          weekday: "long",
        });
  ...
  joinlivestream() {
    ...
    if (this.sharelive === true) {
      window
        .open(
          "https://meet.jit.si/" + md5.appendStr("event:" + this.eventId).end(),
          "_blank"
        )
        .focus();
  ...
  sendGiftCard() {
    window.location.href = "https://prezenty.in/voucher";
  }
}
